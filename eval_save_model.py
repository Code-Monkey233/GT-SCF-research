...
from fl_test import test, load_data, inverse_transform, format_predictions
...
def main():
    ...
    for i in range(len(testloaders)):
        ...
        for round in range(1, ROUND + 1):
            data = dict(np.load(f'./flower/savemodels/round-{round}-weights.npz'))
            npz_keys = []
            for k in data.keys():
                npz_keys.append(k)
...
def evaluate(model, testloader, batch_size = 1, n_features = 1): #返回预测值和
    with torch.no_grad():
        predictions = []
        values = []
